...
plot_ly(
  ...
  layout(
    title = list(text = "💰 Price vs Horsepower", x = 0.5),
...
plot_ly(
  ...
  layout(
    title = list(text = "⚡️ Price vs 0–60 MPH Time", x = 0.5),
    xaxis = list(title = "0–60 MPH Time (Seconds)", gridcolor = "#f0f0f0"),
...
ggplot(car_data, aes(x = `Car Make`, y = `Price (in USD)`, fill = `Car Make`)) +
  geom_boxplot(alpha = 0.8, outlier.color = "blue", outlier.size = 2) +
  scale_y_continuous(labels = dollar_format()) +
  coord_flip() +
  theme_minimal(base_size = 14) +
  labs(
